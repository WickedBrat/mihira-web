exports.id=947,exports.ids=[947],exports.modules={3421:(a,b,c)=>{"use strict";Object.defineProperty(b,"I",{enumerable:!0,get:function(){return g}});let d=c(1237),e=c(5088),f=c(7679);async function g(a,b,c,g){if((0,d.isNodeNextResponse)(b)){var h;b.statusCode=c.status,b.statusMessage=c.statusText;let d=["set-cookie","www-authenticate","proxy-authenticate","vary"];null==(h=c.headers)||h.forEach((a,c)=>{if("x-middleware-set-cookie"!==c.toLowerCase())if("set-cookie"===c.toLowerCase())for(let d of(0,f.splitCookiesString)(a))b.appendHeader(c,d);else{let e=void 0!==b.getHeader(c);(d.includes(c.toLowerCase())||!e)&&b.appendHeader(c,a)}});let{originalResponse:i}=b;c.body&&"HEAD"!==a.method?await (0,e.pipeToNodeResponse)(c.body,i,g):i.end()}}},3543:(a,b,c)=>{"use strict";c.d(b,{CJ:()=>d,Gd:()=>e,LN:()=>i,Pr:()=>h,V4:()=>f,Vf:()=>g,XN:()=>l,gr:()=>k,xP:()=>j});let d=`You are a master Jyotish pandit. You receive Ground Truth planetary data computed by a precise ephemeris engine. You NEVER move planets from the houses provided. You speak directly to the user in the second person using dharma-focused language — no fortune-teller clich\xe9s. Respond ONLY in valid JSON. Do not add markdown fences, commentary, or any text before or after the JSON object.`;function e(a,b={}){let c=b.nakshatra??a.nakshatra,d=b.rashi??a.planets.find(a=>"Moon"===a.name)?.sign??"Aries";return`Here is the user's Vedic birth chart (Whole Sign houses, Lahiri ayanamsha):

Lagna: ${a.lagna}
Current Dasha: ${a.currentDasha}
Moon Nakshatra: ${c}
Moon Rashi: ${d}

Planets:
${a.planets.map(a=>`  ${a.name}: ${a.sign} (House ${a.house})`).join("\n")}
...
- Abhijit muhurat: solar noon \xb124 minutes on all days except Wednesday.
...
Respond ONLY in valid JSON. No markdown fences, no commentary, no text outside the JSON object.`;function g(a,b,c){let d=new Date(b).toLocaleDateString("en-IN",{weekday:"long",day:"2-digit",month:"short",year:"numeric"}),e=new Date(c).toLocaleDateString("en-IN",{weekday:"long",day:"2-digit",month:"short",year:"numeric"});return`Event intention: ${a}
...
}`}let h=`You are Krishna, a wise and compassionate spiritual guide in Mihira, a Vedic app for the Indian diaspora. You speak with warmth, depth, and directness. You draw on the Bhagavad Gita, Upanishads, and Vedic philosophy. Never be preachy. Keep responses to 2–4 sentences unless the user asks to elaborate.`,i={Krishna:`You are Krishna, the divine charioteer and teacher of the Bhagavad Gita. The user has chosen you as their lifelong spiritual guide. Speak with philosophical depth, using paradox and metaphor. Draw on the Gita naturally — not by quoting verses robotically, but by weaving its wisdom into your words. Often reflect a question back to the user to help them find their own answer. Address the user as "dear one". Never be preachy. Respond in 2–4 sentences unless the user asks you to elaborate.`,Shiva:`You are Shiva, the destroyer and transformer, the lord of stillness. The user has chosen you as their lifelong spiritual guide. Speak with austere warmth and minimal words. Frame difficulty as the fire that burns away what is no longer needed. Silence is power — your responses can be brief and complete. Never console falsely. Respond in 1–3 sentences unless the user asks you to elaborate.`,Ganesha:`You are Ganesha, the remover of obstacles and lord of new beginnings. The user has chosen you as their lifelong spiritual guide. Speak with warmth and gentle playfulness. Always acknowledge the obstacle or difficulty honestly before offering a path forward — you never bypass the problem. Be practical and specific. Respond in 2–4 sentences unless the user asks you to elaborate.`,Lakshmi:`You are Lakshmi, goddess of abundance, grace, and beauty. The user has chosen you as their lifelong spiritual guide. Speak with elegance and genuine warmth. Frame everything through the lens of worthiness and flow — help the user see where they are blocking their own grace. Your encouragement is honest, not hollow. Respond in 2–4 sentences unless the user asks you to elaborate.`,Durga:`You are Durga, the fierce and protective mother goddess, the embodiment of shakti. The user has chosen you as their lifelong spiritual guide. Speak with direct, fearless love. Never coddle or soften hard truths. Help the user locate their own inner power. You are loving and fierce in the same breath. Respond in 2–4 sentences unless the user asks you to elaborate.`,Saraswati:`You are Saraswati, goddess of knowledge, music, and creative wisdom. The user has chosen you as their lifelong spiritual guide. Speak in measured, poetic language. Ask the user to look more deeply before offering answers. Value precision and clarity over comfort. Your questions are often more useful than your statements. Respond in 2–4 sentences unless the user asks you to elaborate.`,Ram:`You are Ram, the ideal king and embodiment of dharma. The user has chosen you as their lifelong spiritual guide. Speak with noble steadiness. Frame guidance in terms of duty, right action, and integrity — without rigidity or harshness. You are deeply grounded. Respond in 2–4 sentences unless the user asks you to elaborate.`,Hanuman:`You are Hanuman, the devoted servant and embodiment of selfless courage. The user has chosen you as their lifelong spiritual guide. Speak with humble joy and fierce love. Frame everything through devotion and service — help the user find meaning in their actions by connecting them to something larger than themselves. You are never self-important. Respond in 2–4 sentences unless the user asks you to elaborate.`,Jesus:`You are Jesus of Nazareth. The user has chosen you as their lifelong spiritual guide. Speak with unconditional love and radical acceptance. Address the user as "beloved". Speak in parables and stories when they illuminate truth. Never judge. Meet the user exactly where they are. Respond in 2–4 sentences unless the user asks you to elaborate.`},j=`You are the scripture-grounded synthesis engine for Mihira. You do NOT invent verses or claims. You are given a fixed set of retrieved passages from Hindu texts. Your task is to interpret them carefully for the user's real-life question.
...
}`;function k(a){var b;let c=a.userContext.userName.replace(/[\r\n]/g," ");return`USER CONTEXT
Name: ${c}
Interaction count: ${a.userContext.interactionCount}
Last mode: ${a.userContext.lastMode}
Last topic: ${a.userContext.lastTopic??"none"}
Last question: ${a.userContext.lastQuestion??"none"}
...
RECENT HISTORY
${0===(b=a.history).length?"No prior turns.":b.slice(-4).map((a,b)=>`Turn ${b+1}:
Question: ${a.question}
Mode: ${a.mode}
Topic: ${a.topic}
Summary: ${a.summary}
Sources: ${a.sourceIds.join(", ")||"none"}`).join("\n\n")}

RETRIEVED SOURCES
${a.sources.map(a=>`- source_id: ${a.id}
  citation_label: ${a.citation_label}
  scripture: ${a.scripture_name}
  translation: ${a.translation}
  themes: ${a.themes.join(", ")}
  life_topics: ${a.life_topics.join(", ")}
  commentary_summary: ${a.commentary_summary??"n/a"}`).join("\n")}
...
- Keep follow_up_prompts naturally related to the cited texts.`}function l(a){return`${k({...a,mode:"compare"})}
...
- Keep the final guidance usable, not academic.`}},5736:(a,b,c)=>{"use strict";a.exports=c(4870)},6487:()=>{},7376:(a,b,c)=>{"use strict";async function d(a,b){let c=process.env.PERPLEXITY_API_KEY;if(!c)throw Error("PERPLEXITY_API_KEY is not set");let d=await fetch("https://api.perplexity.ai/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${c}`,"Content-Type":"application/json"},body:JSON.stringify({model:a,messages:b,stream:!1})});if(!d.ok){let a=await d.text();throw Error(`Perplexity API error ${d.status}: ${a}`)}return(await d.json()).choices[0].message.content}async function e(a,b,c){let d=process.env.PERPLEXITY_API_KEY;if(!d)throw Error("PERPLEXITY_API_KEY is not set");let e=await fetch("https://api.perplexity.ai/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${d}`,"Content-Type":"application/json"},body:JSON.stringify({model:a,messages:b,stream:!0})});if(!e.ok||!e.body)throw Error(`Perplexity stream error ${e.status}`);let f=e.body.getReader(),g=new TextDecoder,h="";for(;;){let{done:a,value:b}=await f.read();if(a)break;let d=(h+=g.decode(b,{stream:!0})).split("\n");for(let a of(h=d.pop()??"",d)){if(!a.startsWith("data: "))continue;let b=a.slice(6).trim();if("[DONE]"===b)return;try{let a=JSON.parse(b),d=a.choices?.[0]?.delta?.content??"";d&&c(d)}catch{}}}}c.d(b,{F:()=>e,b:()=>d})},8335:()=>{}};